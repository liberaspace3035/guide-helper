...
<div class="dashboard" x-data="dashboardData()" x-init="init()">
    <div class="dashboard-header">
        <div class="dashboard-title">
            <h1><span x-text="greeting"></span>、<?php echo e($user->name); ?>さん</h1>
            <p class="welcome-message">
                <?php if($user->isUser()): ?>
                    今日も素敵な一日をお過ごしください
                <?php elseif($user->isGuide()): ?>
                    本日もガイド活動をよろしくお願いします
    ...
    <template x-if="announcements.length > 0">
        <section class="announcements-section" aria-label="運営からのお知らせ">
            <div class="section-header">
                ...
                <a href="<?php echo e(route('announcements.index')); ?>" class="view-all-link">
                    すべて見る
                </a>
            </div>
    ...
    <?php if($user->isUser()): ?>
        ...
        <section class="quick-actions">
            <a href="<?php echo e(route('requests.create')); ?>" class="quick-action-btn primary">
            ...
            <a href="<?php echo e(route('requests.index')); ?>" class="quick-action-btn secondary">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <section class="pending-reports-section">
                ...
                <div class="pending-reports-list">
                    <template x-for="report in pendingReports.slice(0, 3)" :key="report.id">
                        <a :href="`<?php echo e(url('/reports')); ?>/${report.id}`" class="pending-report-item" :class="{'revision-requested': report.status === 'revision_requested'}">
                                ...
                                <tr>
                                    ...
                                    <td class="table-actions">
                                        <div class="action-buttons">
                                            <a :href="`<?php echo e(url('/chat')); ?>/${matching.id}`" class="action-btn action-btn-chat" :aria-label="`チャット: ${matching.guide_name}`" :title="`${matching.guide_name}さんとチャット`">
                                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                    <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
                                                </svg>
                                            </a>
                                            <a :href="`<?php echo e(url('/matchings')); ?>/${matching.id}`" class="action-btn action-btn-detail" :aria-label="`詳細: ${matching.guide_name}`" title="詳細を見る">
                                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                    <circle cx="12" cy="12" r="10"></circle>
        ...
        <template x-if="matchings.length === 0">
            <section class="empty-state">
                ...
                <a href="<?php echo e(route('requests.create')); ?>" class="btn-primary">
                    依頼を作成する
                </a>
            </section>
        </template>
    ...
    <?php if($user->isGuide()): ?>
        <!-- ガイド向けダッシュボード -->
        <section class="quick-actions">
            <a href="<?php echo e(route('guide.requests.index')); ?>" class="quick-action-btn secondary">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <line x1="8" y1="6" x2="21" y2="6"></line>
                    <line x1="8" y1="12" x2="21" y2="12"></line>
                    <line x1="8" y1="18" x2="21" y2="18"></line>
            ...
            <template x-if="activeMatchings.length > 0">
                <a :href="`<?php echo e(url('/guide/reports/new')); ?>/${activeMatchings[0].id}`" class="quick-action-btn primary">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
                        <polyline points="14 2 14 8 20 8"></polyline>
            ...
            <section class="pending-reports-section">
                ...
                <div class="pending-reports-list">
                    <template x-for="report in revisionRequestedReports.slice(0, 5)" :key="report.id">
                        <a :href="`<?php echo e(url('/guide/reports/new')); ?>/${report.matching_id}`" class="pending-report-item revision-requested">
                            <div class="report-info">
                                <span class="report-type" x-text="report.request_type"></span>
                                <span class="report-date" x-text="formatDate(report.actual_date)"></span>
                                <span class="report-status-badge revision-badge">修正待ち</span>
                    ...
                    <template x-for="matching in activeMatchings" :key="matching.id">
                        <div class="matching-card">
                            ...
                            <div class="matching-actions">
                                <a :href="`<?php echo e(url('/chat')); ?>/${matching.id}`" class="btn-primary btn-icon">
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                        <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
                                    </svg>
                                    <span>チャット</span>
                                </a>
                                <a :href="`<?php echo e(url('/matchings')); ?>/${matching.id}`" class="btn-secondary btn-icon">
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                        <circle cx="12" cy="12" r="10"></circle>
                                        <line x1="12" y1="16" x2="12" y2="12"></line>
                                        <line x1="12" y1="8" x2="12.01" y2="8"></line>
        ...
        <template x-if="activeMatchings.length === 0">
            <section class="empty-state">
                ...
                <a href="<?php echo e(route('guide.requests.index')); ?>" class="btn-primary">
                    依頼を探す
                </a>
            </section>
        </template>
    ...
    <?php if($user->isAdmin()): ?>
        ...
        <div class="dashboard-cards">
            <div class="card admin-card">
                ...
                <a href="<?php echo e(route('admin.dashboard')); ?>" class="btn-primary btn-icon">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
function dashboardData() {
    return {
        greeting: '',
        notifications: <?php echo json_encode($notifications ?? [], 15, 512) ?>,
        announcements: <?php echo json_encode($announcements ?? [], 15, 512) ?>,
        stats: <?php echo json_encode($stats ?? null, 15, 512) ?>,
        matchings: <?php echo json_encode($matchings ?? [], 15, 512) ?>,
        pendingReports: <?php echo json_encode($pendingReports ?? [], 15, 512) ?>,
        revisionRequestedReports: <?php echo json_encode($revisionRequestedReports ?? [], 15, 512) ?>,
        usageStats: <?php echo json_encode($usageStats ?? null, 15, 512) ?>,
        monthlyLimit: null,
        loadingMonthlyLimit: false,
        get activeMatchings() {
            return this.matchings.filter(m => m.status === 'matched' || m.status === 'in_progress');
        },
        init() {
        ...
        async fetchMonthlyLimit() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    console.error('限度時間取得エラー:', response.status, errorData);
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/maiki/Downloads/ガイドヘルパー/resources/views/dashboard.blade.php ENDPATH**/ ?>